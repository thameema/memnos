## What does this PR do?

<!-- One paragraph. What changed and why. Link related issues with "Fixes #N" or "Closes #N". -->

## Type of change

- [ ] Bug fix
- [ ] New feature
- [ ] Refactor / code quality
- [ ] Documentation
- [ ] Tests
- [ ] Infrastructure / CI

## How was it tested?

<!-- Describe what you ran. Include commands where relevant. -->

```bash
# e.g.
python -m pytest tools/test_api_features.py -v
python -m pytest tools/test_learning.py -v
```

## Checklist

- [ ] All existing tests pass (`pytest tools/test_learning.py tools/test_api_features.py`)
- [ ] New code has tests where meaningful
- [ ] Docstrings / comments updated if behaviour changed
- [ ] `memnos.yaml.example` updated if config schema changed
- [ ] `README.md` or relevant `docs/` file updated if user-facing behaviour changed
- [ ] No credentials, customer names, or Health Chain–specific data included
