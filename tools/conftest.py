...
dataclass.  When the memnos API is not available, all such tests are
...
def runner():
    ...
    Skips the entire test session if the memnos API is not reachable.
    ...
    try:
        with httpx.Client(timeout=5) as c:
            ...
            if r.status_code != 200:
                pytest.skip(
                    f"memnos API not healthy (HTTP {r.status_code}) — "
                    "set MEMNOS_API_URL to override"
                )
    except Exception as exc:
        pytest.skip(
            f"memnos API not reachable at {MEMNOS_API}: {exc} — "
            "set MEMNOS_API_URL to override"
        )
