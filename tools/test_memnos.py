"""
memnos v0.2 pytest test suite — REST API coverage.
...
Export env vars before running (defaults shown):
    MEMNOS_BASE_URL=http://127.0.0.1:8766
    MEMNOS_API_KEY=memnos-local-dev-key

Run:
    python -m pytest tools/test_memnos.py -v
    python -m pytest tools/test_memnos.py -v -m "not slow"   # skip entity tests
"""
...
BASE_URL: str = os.environ.get("MEMNOS_BASE_URL", "http://127.0.0.1:8766").rstrip("/")
API_KEY: str = os.environ.get("MEMNOS_API_KEY", "memnos-local-dev-key")
...
TEST_NS: str = f"test:memnos:{_RUN_ID}"
...
class TestHealthCheck:
    ...
    def test_root_responds(self, client: httpx.Client) -> None:
        """GET / should return a JSON body identifying the service as 'memnos'."""
        resp = client.get("/")
        assert resp.status_code == 200, f"Expected 200, got {resp.status_code}: {resp.text}"
        body = resp.json()
        assert body.get("service") == "memnos", (
            f"Expected service='memnos', got: {body}"
        )
...
class TestMemoryWrite:
    ...
    def test_write_returns_id(self, client: httpx.Client) -> None:
        ...
        data = write_memory(client, "memnos write test — basic field check", ns, tags=["test"])
