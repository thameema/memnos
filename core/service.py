...
# --- PINNED-CONSTRAINT SUBJECTS (issue #153) -----------------------------------------
...
# every prompt. Pinned constraints are deliberately UNBOUNDED — no write-time size/count
# guard, no cap. They are curated governance rules, not ranked content, and are never
# subject to a budget (see render_context: pinned rows always render in full). The actual
# fix for the incident is giving every constraint a subject so an untagged one is never
# unretirable, and the render_context skip-vs-stop fix below.
...
class MemnosMemory:
    ...
    def remember_turn(self, namespace: str, text: str, *, speaker=None, session_id=None,
        ...
        Pinned constraints are unbounded — no write-time size/count guard; legacy
        untagged live constraints in this namespace are backfilled with a derived
        subject on every constraint write, so they become retirable too."""
        observed_at = observed_at or datetime.now(timezone.utc)
        if self.redact:
    ...
    def render_context(rows, *, max_chars=9000, viewer=None, query=None) -> str:
        ...
        for r in rows:
            ...
            # issue #153: a PINNED constraint always renders in full and NEVER counts
            # against max_chars — pins are additive, outside the budget entirely, not
            # merely exempt from being dropped. max_chars is the budget for RANKED
            # content (facts/turns) only. This is deliberate: the production incident was
            # 6 pins alone exceeding the budget and starving every fact to zero — making
            # pins "always render but still consume used" would reproduce that exact
            # failure with a bigger pinned set. Non-pinned rows SKIP a row that doesn't
            # fit rather than stopping (`break` let one oversized row block every shorter
            # row after it, even with most of the budget still free). Precedence is
            # unchanged: rows are still considered in order (pins, then facts, then
            # turns), so an earlier non-pinned row always claims space before a later one.
            if r.get("pinned"):
                out.append(line)
                continue
