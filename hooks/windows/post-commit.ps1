# post-commit.ps1
#
# Global git post-commit hook — auto-writes every git commit to memnos memory.
...
# it is called after memnos write so repo-specific logic can still run.
#
...
# ── Load config ────────────────────────────────────────────────────────────────
$EnvFile = Join-Path $HOME ".claude\hooks\memnos.env"

$MEMNOS_API        = "http://localhost:8766"
$MEMNOS_KEY        = "memnos-local-dev-key"
...
try {
    ...
    exit 0   # memnos not running — never block a commit
}
...
try {
    $DotMemnos = Join-Path $RepoRoot ".memnos"
...
$Content = "[memnos-commit] $CommitMsg`nrepo: $RepoName | commit: $CommitHash | branch: $Branch | author: $CommitAuthor`nfiles: $ChangedFiles"
