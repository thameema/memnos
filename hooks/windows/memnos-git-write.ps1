# memnos-git-write.ps1
#
# Claude Code PostToolUse hook — two jobs:
# 1. Git commits: written to memnos immediately when Claude runs git commit
...
#   "command": "powershell.exe -NonInteractive -NoProfile -File \"C:\\path\\to\\memnos-git-write.ps1\""
...
if (Test-Path $EnvFile) {
    foreach ($line in (Get-Content $EnvFile)) {
        ...
        switch ($key) {
            ...
            "MEMNOS_AUTOSAVE_MINUTES" { $SAVE_INTERVAL_MINUTES = [int]$value }
        }
    }
}
...
if ($ToolName -eq "Bash" -and $Cmd -match "git commit") {
    ...
    try {
        $null = Invoke-RestMethod `
            -Uri "$MEMNOS_API/api/v1/memory/" `
            -Method Post `
            -Headers @{ "Content-Type" = "application/json"; "Authorization" = "Bearer $MEMNOS_KEY"; "X-Memnos-Tool" = "post-tool-hook" } `
...
$CounterFile  = Join-Path $env:TEMP "memnos_counter_$SessionId"
$LastSaveFile = Join-Path $env:TEMP "memnos_lastsave_$SessionId"
...
if ($Elapsed -ge ($SAVE_INTERVAL_MINUTES * 60)) {
    ...
    if ((Test-Path $Transcript) -and (Get-Command claude -ErrorAction SilentlyContinue)) {
        ...
        $bgScript = {
            param($transcript, $project, $branch, $count, $memnosApi, $memnosKey, $ns, $sessionId)
            ...
            try {
                $null = Invoke-RestMethod `
                    -Uri "$memnosApi/api/v1/memory/" `
                    -Method Post `
                    -Headers @{ "Content-Type" = "application/json"; "Authorization" = "Bearer $memnosKey"; "X-Memnos-Tool" = "periodic-autosave" } `
                    -Body ([System.Text.Encoding]::UTF8.GetBytes($payload)) `
                    -TimeoutSec 5
            } catch { }
        }
        ...
        Start-Job -ScriptBlock $bgScript -ArgumentList `
            ...
            $MEMNOS_API, $MEMNOS_KEY, $EngNS, $SessionId | Out-Null
    }
}
