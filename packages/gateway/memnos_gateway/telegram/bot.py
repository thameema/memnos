"""
memnos_gateway.telegram.bot — Telegram bot gateway using python-telegram-bot v21.

The bot accepts natural-language messages and routes them through the memnos
...
from memnos_gateway.telegram.formatter import (
    format_result,
...
try:
    from memnos.models import MemoryType as _MemoryType
...
class TelegramGateway:
    """
    Telegram bot gateway that forwards user messages to the memnos orchestrator.
    ...
    orchestrator:
        memnos Orchestrator instance.
    client:
        MemnosClient instance (used for direct memory commands).
    default_namespace:
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Process an incoming text message through the memnos orchestrator."""
        ...
        try:
            from memnos_learning.feedback import detect_correction, FeedbackService  # type: ignore
        ...
        if len(result_text) <= _MAX_MSG_LEN:
            ...
            filename = f"memnos_result_{timestamp}.txt"
    ...
    async def handle_feedback_callback(
        ...
        if thumbs_up is not None:
            try:
                from memnos_learning.feedback import FeedbackService  # type: ignore
                from memnos_learning.episode_store import EpisodeStore  # type: ignore
                from memnos_learning.quality_store import QualityStore  # type: ignore
                ep_store = EpisodeStore()
                await ep_store.init()
