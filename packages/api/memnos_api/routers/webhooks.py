"""
memnos_api.routers.webhooks — Incident webhook receiver.
...
Authentication
--------------
Optional header X-Memnos-Webhook-Secret must match MEMNOS_WEBHOOK_SECRET env var
...
_WEBHOOK_SECRET = os.environ.get("MEMNOS_WEBHOOK_SECRET", "")
...
def _verify_webhook_secret(secret_header: str | None) -> None:
    ...
    if not secret_header:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Missing X-Memnos-Webhook-Secret")
...
async def receive_incident(
    request: Request,
    namespace: str = _DEFAULT_NAMESPACE,
    x_memnos_webhook_secret: str | None = Header(default=None),
    ...
    """
    _verify_webhook_secret(x_memnos_webhook_secret)
...
async def resolve_incident(
    ...
    from memnos.models import MemoryType
