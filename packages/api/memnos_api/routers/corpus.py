"""
memnos_api.routers.corpus — Architecture corpus ingestion and constraint checking.
...
on the gridops-platform repo so constraint nodes stay current on every doc push.
"""
...
from memnos_api.auth import get_client, require_api_key, require_api_key_entry
...
async def _run_sync(corpus: Corpus, client, store) -> None:
    """Extract nodes from corpus source and write them to memnos. Background task."""
    corpus_id = corpus.id
