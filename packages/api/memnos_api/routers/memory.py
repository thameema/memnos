"""
memnos_api.routers.memory — CRUD and search endpoints for persistent memory.
...
from memnos.models import Episode, MemoryStatus, MemoryType, Provenance
...
from memnos_api.schemas import MemoryResponse, MemoryWriteRequest, ReviewDueItem
...
def _detect_git_commit() -> str:
    ...
    # 1. Explicit env var — set this in docker-compose or CI: MEMNOS_GIT_COMMIT=abc1234
    commit = os.environ.get("MEMNOS_GIT_COMMIT", "").strip()
    if commit:
        return commit
...
def _to_response(memory, score: float | None = None) -> MemoryResponse:
    ...
    return MemoryResponse(
        ...
        source_episode_ids=list(getattr(memory, "source_episode_ids", None) or []),
...
async def write_memory(
    ...
    if not prov.get("tool"):
        prov["tool"] = request.headers.get("X-Memnos-Tool", "api")
    if not prov.get("agent_id"):
        prov["agent_id"] = request.headers.get("X-Memnos-Agent-Id", "")
    ...
    from memnos.contradiction.detector import check_contradictions
    pre_warnings: list = []
    ...
    # ── Feature 2: write the immutable Episode FIRST so the Memory carries
    #              source_episode_ids pointing back to the verbatim input.
    #              Best-effort — if Episode insert fails (eg. legacy DB without
    #              the Episode type yet), we still write the Memory.
    ...
    try:
        ...
        logger.debug("Auto-created Episode %s for new Memory in ns=%s", ep.id, req.namespace)
    except Exception as exc:
        logger.warning("Auto-create Episode failed (non-fatal — Memory still written): %s", exc)
...
# ---------------------------------------------------------------------------
# Contradiction dry-run (Feature 7)
# ---------------------------------------------------------------------------
...
async def check_contradictions_dryrun(
    req: CheckContradictionsRequest,
    user_id: str = Depends(require_api_key),
    key_entry=Depends(require_api_key_entry),
    client=Depends(get_client),
) -> dict:
    """Preview contradictions for *content* WITHOUT writing it.

    Lets agents and human reviewers see which existing memories would be
    auto-superseded (or surface as warnings) if this content were written,
    so they can edit/cancel before persisting.

    Response::

        {
          "would_supersede":   [ {existing_id, similarity, direction, content}, ... ],
          "warnings_only":     [ {existing_id, similarity, direction, content}, ... ],
          "candidate_content": "...",
          "namespace":         "...",
          "memory_type":       "..."
        }

    ``would_supersede`` lists directional conflicts (negation, opposite
    polarity, llm-confirmed, high-similarity flips) that the live write
    path auto-supersedes. ``warnings_only`` lists similarity_only matches —
    likely related but not auto-actioned.
    """
    from memnos.contradiction.detector import check_contradictions

    await check_namespace_access(key_entry, req.namespace, operation="read")
    try:
        warnings = await check_contradictions(
            client, req.content, req.namespace, req.memory_type, req.tags,
        )
    except Exception as exc:
        logger.exception("check-contradictions failed: %s", exc)
        raise HTTPException(status_code=500, detail=str(exc)) from exc

    would_supersede: list[dict] = []
    warnings_only: list[dict] = []
    for w in warnings:
        directional = w.direction in (
            "negation_detected", "opposite_polarity", "topic_update", "llm_confirmed",
        )
        high_sim_flip = w.direction == "similarity_only" and w.similarity >= 0.75
        bucket = would_supersede if (directional or high_sim_flip) else warnings_only
        bucket.append({
            "existing_id": w.existing_id,
            "existing_content": w.existing_content[:200],
...
async def search_memory(
    ...
    mode: str = Query("hybrid", description="hybrid | vector | graph | bm25 | rrf"),
    min_confidence: float = Query(
        0.0, ge=0.0, le=1.0,
        description="Feature 5 — drop results with confidence < this value",
    ),
    include_reference: bool = Query(
        False,
        description=(
            "Feature 8 — include results from namespaces typed as 'reference' "
            "(org knowledge base, standards, policies). Default false — "
            "reference content stays out of operational queries unless asked for."
        ),
    ),
    ...
    if is_all:
        ...
        try:
            results = await client.search(q, "all", top_k * 4, mode=mode, as_of=as_of, include_reference=include_reference)
        except Exception as exc:
        ...
        try:
            results = await client.search(q, _ns_raw, top_k, mode=mode, as_of=as_of, include_reference=include_reference)
        except Exception as exc:
    ...
    if min_confidence > 0.0:
        results = [
            r for r in results
            if float(getattr(r.memory, "confidence", 1.0) or 1.0) >= min_confidence
        ]
...
async def get_memory(
    ...
    try:
        # MemnosClient exposes get_memory(); fall back to get() for compatibility
