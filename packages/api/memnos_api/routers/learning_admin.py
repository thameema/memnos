"""
memnos_api.routers.learning_admin — Learning admin dashboard API.
...
from memnos_api.auth import check_namespace_access, require_api_key_entry
    ...
    if store is None:
        raise HTTPException(
            ...
            detail=(
                ...
                "Install memnos-learning: pip install memnos-learning"
            ),
        )
    return store
...
async def trigger_reflection(
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail="memnos-learning package is not installed",
        )
    except Exception as exc:
...
<head>
<meta charset="UTF-8">
<title>memnos — Learning Admin</title>
<style>
...
window.onload = () => {
  const k = sessionStorage.getItem('memnos_key');
