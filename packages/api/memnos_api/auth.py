"""
memnos_api.auth — FastAPI dependency-injection helpers for authentication and shared state.

Provides:
    get_config()             — returns app.state.config (MemnosConfig)
    get_client()             — returns app.state.client (MemnosClient)
...
from memnos.config import ApiKeyEntry  # type: ignore
...
def get_config(request: Request):
    """Return the MemnosConfig singleton stored on app.state."""
    return request.app.state.config


def get_client(request: Request):
    """Return the MemnosClient singleton stored on app.state."""
