"""
memnos.cli.community_cli — memnos-community CLI entry point.

Usage
-----
    memnos-community detect --namespace org:my-team
    memnos-community detect --namespace "*"
    memnos-community detect --namespace org:my-team --min-size 3
    memnos-community list --namespace org:my-team
"""
...
async def _detect(args: argparse.Namespace) -> int:
    ...
    try:
        cfg = MemnosConfig.from_yaml(args.config) if args.config else MemnosConfig.load()
    except Exception:
        cfg = None
...
async def _list(args: argparse.Namespace) -> int:
    ...
    if not communities:
        print(f"No communities found for namespace: {args.namespace}")
        print("Run 'memnos-community detect' to build communities.")
        return 0
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="memnos-community",
        description="memnos community detection — find entity clusters in the knowledge graph.",
    )
    parser.add_argument(
        "--config",
        default=None,
        help="Path to memnos YAML config file (default: auto-detect from $MEMNOS_CONFIG)",
    )
