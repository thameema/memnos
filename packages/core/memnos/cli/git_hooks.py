"""
memnos.cli.git_hooks — memnos-git CLI: install and run git hooks.

Commands
--------
memnos-git install [--repo PATH] [--server URL]
    Install post-commit and pre-review hooks into a git repository.

memnos-git post-commit [--repo PATH] [--server URL] [--namespace NS]
    Run the post-commit hook manually (also called by the installed hook).

memnos-git pre-review [--diff PATH] [--server URL] [--namespace NS]
    Given a diff file or stdin, retrieve relevant memnos memories as context.
"""
...
_DEFAULT_SERVER = os.environ.get("MEMNOS_SERVER_URL", "http://localhost:8766")
_DEFAULT_NAMESPACE = os.environ.get("MEMNOS_NAMESPACE", "org:default")
_HOOK_TEMPLATE = textwrap.dedent(
    """\
    #!/bin/sh
    # memnos post-commit hook — writes a memory for every commit
    # Installed by: memnos-git install
    memnos-git post-commit --server {server} --namespace {namespace}
    """
)
_INCIDENT_HOOK_TEMPLATE = textwrap.dedent(
    """\
    #!/bin/sh
    # memnos post-merge hook — records incident RCA when an incident branch is merged
    # Installed by: memnos-git install --incident
    memnos-git post-merge-incident --server {server} --namespace {namespace}
    """
)
...
def cmd_install(args: argparse.Namespace) -> int:
    ...
    print(f"  Every commit will write a memory to memnos automatically.")
...
def cmd_post_commit(args: argparse.Namespace) -> int:
    ...
    api_key = os.environ.get("MEMNOS_API_KEY", "")
...
async def _post_memory(server: str, headers: dict, payload: dict) -> int:
    try:
        ...
        async with httpx.AsyncClient(timeout=10) as client:
            ...
            data = resp.json()
            print(f"memnos: commit memory written (id: {data.get('id', '?')})", file=sys.stderr)
            return 0
    except Exception as exc:
        # Non-fatal — git commits should never fail due to memnos being down
        print(f"memnos: post-commit hook skipped ({exc})", file=sys.stderr)
        return 0
...
def cmd_post_merge_incident(args: argparse.Namespace) -> int:
    ...
    if rc == 0:
        print(
            f"memnos: incident RCA memory written for {merged_branch!r} (severity: {severity})",
            file=sys.stderr,
        )
    return rc
...
def cmd_pre_review(args: argparse.Namespace) -> int:
    ...
    if results:
        print("\n=== memnos: relevant memories for this diff ===")
        print(results)
        print("=================================================\n")
    return 0
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="memnos-git",
        description="memnos git integration — install hooks and surface memories during development",
    )
    ...
    p_install.add_argument("--server", default=_DEFAULT_SERVER, help="Memnos server URL")
    ...
    p_post.add_argument("--server", default=_DEFAULT_SERVER, help="Memnos server URL")
    ...
    p_merge.add_argument("--server", default=_DEFAULT_SERVER, help="Memnos server URL")
    ...
    p_review.add_argument("--server", default=_DEFAULT_SERVER, help="Memnos server URL")
