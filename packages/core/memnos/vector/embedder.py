"""
memnos.vector.embedder — Embedding abstraction with multiple backends.
...
local              — sentence-transformers (CPU/GPU, fully offline).
                     Opt-in: pip install 'memnos-core[local-embeddings]'
...
if TYPE_CHECKING:
    from memnos.config import EmbeddingsConfig
...
class FastEmbedEmbedder(Embedder):
    """
    Embedder backed by FastEmbed (ONNX runtime, CPU-only, fully offline).

    Default local embedder as of v1.6 — strictly better than sentence-transformers
    + torch on every relevant axis for enterprise installs:

      * ~150 MB install vs ~2 GB for sentence-transformers + torch + CUDA
      * Faster CPU inference (ONNX graph optimizations)
      * Native ARM64 (Apple Silicon, AWS Graviton)
      * Zero GPU/CUDA driver dependency
      * Supported models include nomic-embed-text-v1.5 (768-dim, retrieval-tuned,
        quality competitive with OpenAI text-embedding-3-small) and
        sentence-transformers/all-MiniLM-L6-v2 (384-dim, drop-in compat with
        memnos <= v1.5 installs that already have 384-dim vectors on disk).

    Requires: pip install fastembed
    """

    # Model name → vector dimension. FastEmbed will download on first use and cache.
    ...
    def __init__(self, model: str = "nomic-ai/nomic-embed-text-v1.5") -> None:
        # Tolerate the bare 'all-MiniLM-L6-v2' name from pre-v1.6 configs.
        ...
        if self._model is None:
            try:
                ...
                raise ImportError(
                    ...
                    "Enable local embeddings with: pip install fastembed\n"
                    "(~150 MB — ONNX runtime, no torch, no CUDA)."
                ) from exc
            logger.info("FastEmbedEmbedder: loading %s (first call — downloads model if cold)",
                        self._model_name)
    ...
    @property
    def vector_size(self) -> int:
        # Prefer the static lookup — avoids loading the model just to ask its dim.
        dim = self._KNOWN_DIMS.get(self._model_name)
        if dim:
            return dim
        # Unknown model: load it and inspect via a single test embedding.
        try:
            sample = list(self._get_model().embed(["dim probe"]))  # type: ignore[union-attr]
            return int(len(sample[0]))
        except Exception:
            return 768  # nomic-embed-text-v1.5 default
    ...
    async def embed_batch(self, texts: list[str]) -> list[list[float]]:
        ...
        logger.debug("FastEmbedEmbedder: embedding %d texts with %s",
                     len(texts), self._model_name)
        loop = asyncio.get_event_loop()
        # fastembed returns a generator of numpy arrays
        return await loop.run_in_executor(
            None,
            lambda: [vec.tolist() for vec in model.embed(texts)],  # type: ignore[union-attr]
        )


class LocalEmbedder(Embedder):
    """
    Legacy embedder backed by sentence-transformers (CPU/GPU, fully offline).

    Kept as a fallback for users who specifically need a sentence-transformers
    model that FastEmbed doesn't support. New installs default to FastEmbedEmbedder
    (smaller image, faster CPU inference, ARM64 native).

    Opt-in: pip install 'memnos-core[local-embeddings-st]'
    Pulls in torch + CUDA (~2 GB). Not included in the default Docker image.
    """
        ...
        if self._model is None:
            try:
                ...
                raise ImportError(
                    ...
                    "For most users, FastEmbed (ONNX) is a better choice — try setting\n"
                    "  embeddings.provider: local\n"
                    "  embeddings.model: nomic-ai/nomic-embed-text-v1.5\n"
                    "and install fastembed instead (~150 MB vs ~2 GB).\n"
                    "If you specifically need a sentence-transformers model, install:\n"
                    "  pip install 'memnos-core[local-embeddings-st]'  (~2 GB)"
                ) from exc
...
def get_embedder(config: "EmbeddingsConfig") -> Embedder:
    ...
    if provider == "local":
        # config.model may contain an API model name when auto-detect fell back
        # to local — pick a sensible default in that case.
        _api_models = {"text-embedding-3-small", "text-embedding-3-large", "text-embedding-ada-002"}
        local_model = config.model if config.model and config.model not in _api_models else "nomic-ai/nomic-embed-text-v1.5"

        # Prefer FastEmbed (ONNX, ~150 MB, faster, ARM64-native). Fall back
        # to sentence-transformers (~2 GB) only if FastEmbed isn't installed.
        try:
            import fastembed  # type: ignore # noqa: F401
            return FastEmbedEmbedder(model=local_model)
        except ImportError:
            logger.warning(
                "fastembed not installed — falling back to sentence-transformers. "
                "Install fastembed for a much lighter local-embeddings image."
            )
            # ST doesn't accept the 'nomic-ai/' or 'BAAI/' prefixes its sister
            # FastEmbed uses for the same models, so coerce to a known ST default
            # when we get a FastEmbed-prefixed name.
            if "/" in local_model and local_model.startswith(("nomic-ai/", "BAAI/", "intfloat/", "thenlper/")):
                local_model = "all-MiniLM-L6-v2"
            return LocalEmbedder(model=local_model)

    # Explicit opt-in to the legacy sentence-transformers path when a user
    # specifically wants a model FastEmbed doesn't support.
    if provider in ("local-st", "sentence-transformers"):
        return LocalEmbedder(model=config.model or "all-MiniLM-L6-v2")

    if provider == "local-fastembed":
        return FastEmbedEmbedder(model=config.model or "nomic-ai/nomic-embed-text-v1.5")

    raise ValueError(
        f"Unknown embeddings provider {config.provider!r}. "
        "Supported: 'auto', 'openai', 'openrouter', 'voyage', 'local', "
        "'local-fastembed', 'local-st'."
    )
