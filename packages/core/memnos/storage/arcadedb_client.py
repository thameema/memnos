"""
memnos.storage.arcadedb_client — Async ArcadeDB client for memnos v0.2.
...
from memnos.time import from_epoch_ms, now_ms, to_epoch_ms
...
class ArcadeDBClient:
    """Async ArcadeDB REST client for memnos."""
    ...
    async def _ensure_database(self) -> None:
        """Create the memnos database if it does not exist."""
        try:
    ...
    async def _init_schema(self) -> None:
        ...
        schema_cmds = [
            ...
            "CREATE VERTEX TYPE Episode IF NOT EXISTS",  # Feature 2 — immutable raw input
            # Edge types
            ...
            # Feature 2 — source episode lineage on Memory
            "CREATE PROPERTY Memory.source_episode_ids IF NOT EXISTS LIST",
            # Feature 5 — per-memory confidence score [0.0, 1.0]
            "CREATE PROPERTY Memory.confidence IF NOT EXISTS FLOAT",
            # Feature 8 — Namespace vertex carries scope metadata (work vs reference).
            # Default behaviour: any namespace WITHOUT a NamespaceMeta vertex is
            # treated as "work" — backward compatible with all existing data.
            "CREATE VERTEX TYPE NamespaceMeta IF NOT EXISTS",
            "CREATE PROPERTY NamespaceMeta.name IF NOT EXISTS STRING",
            "CREATE PROPERTY NamespaceMeta.namespace_type IF NOT EXISTS STRING",
            "CREATE INDEX IF NOT EXISTS ON NamespaceMeta (name) UNIQUE",
            # Feature 2 — Episode (immutable raw input) properties + index
            "CREATE PROPERTY Episode.id IF NOT EXISTS STRING",
            "CREATE PROPERTY Episode.content IF NOT EXISTS STRING",
            ...
            "CREATE PROPERTY Episode.author IF NOT EXISTS STRING",
            "CREATE PROPERTY Episode.metadata IF NOT EXISTS MAP",
            "CREATE PROPERTY Episode.provenance IF NOT EXISTS MAP",
            "CREATE INDEX ON Episode (id) IF NOT EXISTS UNIQUE",
            "CREATE INDEX ON Episode (namespace) IF NOT EXISTS NOTUNIQUE",
    ...
    async def insert_episode(self, episode: Episode) -> str:
        ...
        Episodes are immutable — no `update_episode` or `delete_episode`
        method is intentionally exposed.
        """
        sql = (
            "INSERT INTO Episode SET "
            "id = :id, content = :content, namespace = :namespace, "
            "created_at = :created_at, source = :source, author = :author, "
        ...
        params = {
            ...
            "provenance": episode.provenance.model_dump() if episode.provenance else {},
        }
        await self._command(sql, params)
    ...
    async def list_memories_for_episode(
        self,
        episode_id: str,
        namespace: str,
    ) -> list[MemoryEntry]:
        """Return every Memory that was derived from a given Episode."""
        sql = (
            "SELECT * FROM Memory WHERE namespace = :ns "
    ...
    async def _get_candidate_rows(
        ...
        if as_of is not None:
            ...
            rows = await self._query(
                ...
                f"expires_at, review_by, provenance, content_embedding, source_episode_ids, confidence "
        ...
        rows = await self._query(
            ...
            "expires_at, review_by, provenance, content_embedding, source_episode_ids, confidence "
    ...
    async def graph_search(
        ...
        """
        from memnos.extraction.spacy_extractor import get_extractor
    ...
    # ------------------------------------------------------------------
    # Feature 8 — NamespaceMeta (work vs reference)
    # ------------------------------------------------------------------

    async def set_namespace_type(self, name: str, namespace_type: str) -> None:
        """Set or update the type of a namespace ("work" | "reference").

        Upsert semantics: if the NamespaceMeta vertex exists, update its type;
        otherwise insert it. Invalidates the reference-namespace cache so the
        next search reflects the change immediately.
        """
        if namespace_type not in ("work", "reference"):
            raise ValueError(f"namespace_type must be 'work' or 'reference', got {namespace_type!r}")
        existing = await self._query(
            "SELECT FROM NamespaceMeta WHERE name = :name LIMIT 1",
            {"name": name},
        )
        if existing:
            await self._command(
                "UPDATE NamespaceMeta SET namespace_type = :nt WHERE name = :name",
                {"nt": namespace_type, "name": name},
            )
        else:
            await self._command(
                "INSERT INTO NamespaceMeta SET name = :name, namespace_type = :nt",
                {"name": name, "nt": namespace_type},
            )
        # Bust the cache so search picks up the change next time
        self._ns_type_cache = None
        self._ns_type_cache_ts = 0.0

    async def get_namespace_type(self, name: str) -> str:
        """Return the namespace's type. Defaults to 'work' if no metadata
        vertex exists — backward-compatible with all existing namespaces."""
        rows = await self._query(
            "SELECT namespace_type FROM NamespaceMeta WHERE name = :name LIMIT 1",
            {"name": name},
        )
        if rows and rows[0].get("namespace_type"):
            return str(rows[0]["namespace_type"])
        return "work"

    async def list_reference_namespaces(self) -> set[str]:
        """Return the set of namespace names typed as 'reference'.

        Cached for 30 seconds — the search path calls this on every query to
        filter out reference content from default results. The cache is busted
        on any set_namespace_type call.
        """
        import time as _t
        now = _t.monotonic()
        if (
            getattr(self, "_ns_type_cache", None) is not None
            and (now - getattr(self, "_ns_type_cache_ts", 0.0)) < 30.0
        ):
            return self._ns_type_cache
        try:
            rows = await self._query(
                "SELECT name FROM NamespaceMeta WHERE namespace_type = 'reference'",
            )
            refs = {str(r["name"]) for r in rows if r.get("name")}
        except Exception as exc:
            logger.warning("list_reference_namespaces failed: %s", exc)
            refs = set()
        self._ns_type_cache = refs
        self._ns_type_cache_ts = now
        return refs

    async def list_namespaces_with_types(self) -> list[dict]:
        """Return [{name, namespace_type}] for every known namespace.

        Combines all distinct namespaces found in Memory rows with the
        NamespaceMeta vertex data. Anything not in NamespaceMeta defaults to
        'work'.
        """
        all_ns = await self.list_namespaces()
        ref_ns = await self.list_reference_namespaces()
        return [
            {"name": ns, "namespace_type": ("reference" if ns in ref_ns else "work")}
            for ns in all_ns
        ]
    ...
    async def search_memories(
        ...
        if len(namespaces) == 1:
            ...
            # (embedding is done at the MemnosClient layer; here we call vector_search
...
def _row_to_memory(row: dict) -> MemoryEntry:
    from memnos.models import MemoryType, MemoryStatus, DecayPolicy
