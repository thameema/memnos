"""
memnos.client — MemnosClient: the primary public API for memnos.

Usage (async context manager):

    async with MemnosClient(config) as client:
...
from memnos.extraction.llm_extractor import get_llm_extractor, LLMExtractor
...
from memnos.storage.vector_backend import VectorBackend, create_vector_backend
from memnos.vault.secret_detector import detect as _detect_secrets, redact as _redact_secrets
from memnos.vault.vault_client import get_vault_client
from memnos.vector.embedder import get_embedder
...
class MemnosClient:
    """Primary public API for memnos persistent memory.
    ...
    def __init__(self, config: MemnosConfig) -> None:
        self._config = config
    ...
    async def start(self) -> None:
        ...
        if self._started:
            logger.debug("MemnosClient.start() called but already started — skipping")
            return
        logger.info("Starting MemnosClient")
        await self._arcadedb.init()
        self._vector_backend = create_vector_backend(self._embedder.vector_size)
        self._started = True
        logger.info("MemnosClient ready")

    async def stop(self) -> None:
        """Close all backend connections."""
        logger.info("Stopping MemnosClient")
        ...
        self._started = False
        logger.info("MemnosClient stopped")

    # ------------------------------------------------------------------
    # Async context manager support
    # ------------------------------------------------------------------

    async def __aenter__(self) -> "MemnosClient":
        await self.start()
        return self
    ...
    def _assert_started(self) -> None:
        if not self._started:
            raise RuntimeError(
                "MemnosClient must be started before use. "
                "Call `await client.start()` or use it as `async with MemnosClient(...) as client:`."
            )
    ...
    async def add(
        ...
        # Feature 5 — pick per-source default when caller did not specify
        from memnos.models import default_confidence_for_source
        resolved_confidence = (
            float(confidence) if confidence is not None
            else default_confidence_for_source(source)
        )
        memory = MemoryEntry(
            content=content,
            namespace=namespace,
    ...
    async def _dispatch_immediate(self, memory: MemoryEntry, namespace: str) -> None:
        ...
        try:
            from memnos.subscription_bus import publish as _publish
        except ImportError:
            return
            ...
            try:
                async with _httpx.AsyncClient(timeout=10.0) as http:
                    resp = await http.post(
                        sub["webhook_url"],
                        json=payload,
                        headers={"Content-Type": "application/json", "X-Memnos-Event": "memory.created"},
                    )
    ...
    async def _fanout_memory(
        ...
        for sub in subscribers:
            ...
            from memnos.models import MemoryEntry as _ME
            copy = _ME(
    ...
    async def search(
        ...
        self._assert_started()
        logger.debug(
            "search: query=%r namespace=%s top_k=%d historical=%s mode=%s as_of=%s ref=%s",
            query, namespace, top_k, include_historical, mode, as_of, include_reference,
        )
        if mode == "graph":
            r = await self._arcadedb.graph_search(
                query=query, namespace=namespace, top_k=top_k,
                include_superseded=include_historical, as_of=as_of,
            )
            return await self._strip_reference(r, include_reference)

        # Feature 3 — lexical-only BM25 over namespace contents.
        if mode == "bm25":
            r = await self._bm25_search(
                query=query, namespace=namespace, top_k=top_k,
                include_historical=include_historical,
            )
            return await self._strip_reference(r, include_reference)

        # Feature 3 — Reciprocal Rank Fusion of vector + BM25 rankings.
        if mode == "rrf":
            r = await self._rrf_search(
                query=query, namespace=namespace, top_k=top_k,
                include_historical=include_historical, as_of=as_of,
                _precomputed_embedding=_precomputed_embedding,
            )
            return await self._strip_reference(r, include_reference)
        ...
        return await self._strip_reference(results, include_reference)

    # ------------------------------------------------------------------
    # Feature 8 — Reference-namespace filter
    # ------------------------------------------------------------------

    async def _strip_reference(
        self, results: "list[SearchResult]", include_reference: bool,
    ) -> "list[SearchResult]":
        """Drop memories from any namespace typed as 'reference' unless the
        caller explicitly opted in. This is the chokepoint that prevents
        knowledge-base material (CMS standards, HL7 IGs, org policies) from
        bleeding into operational-memory queries.
        """
        if include_reference:
            return results
        try:
            ref = await self._arcadedb.list_reference_namespaces()
        except Exception as exc:
            logger.warning("reference-ns lookup failed (returning unfiltered): %s", exc)
            return results
        if not ref:
            return results
        return [r for r in results if r.memory.namespace not in ref]

    # ------------------------------------------------------------------
    # Feature 3 — BM25 + RRF hybrid search
    # ------------------------------------------------------------------

    async def _scan_for_search(
        self, namespace: str, include_historical: bool, cap: int = 5000,
    ) -> list:
        """Pull memories for lexical scoring. Caps at *cap* to bound latency."""
        if namespace in ("all", "*", ""):
            db_namespaces = await self._arcadedb.list_namespaces()
            collected: list = []
            for ns in db_namespaces:
                if len(collected) >= cap:
                    break
                rows = await self._arcadedb.scan_namespace(
                    ns, batch_size=500, include_superseded=include_historical,
                )
                collected.extend(rows)
            return collected[:cap]
        return await self._arcadedb.scan_namespace(
            namespace, batch_size=500, include_superseded=include_historical,
        )

    async def _bm25_search(
        self, query: str, namespace: str, top_k: int, include_historical: bool,
    ) -> "list[SearchResult]":
        from memnos.bm25 import BM25Index
        memories = await self._scan_for_search(namespace, include_historical)
        if not memories:
            return []
        idx = BM25Index([(str(m.id), m.content) for m in memories])
        ranked = idx.score(query)[:top_k]
        by_id = {str(m.id): m for m in memories}
        return [
            SearchResult(
                memory=by_id[mid], score=float(score), source="bm25",
                is_current=(by_id[mid].superseded_at is None),
            )
    ...
    async def _rrf_search(
        ...
        """Vector + BM25 fused via Reciprocal Rank Fusion (k=60).

        Both arms request ``top_k * 3`` candidates to give RRF enough overlap.
        Score returned is the fused RRF score, not the original similarity.
        """
        from memnos.bm25 import BM25Index, reciprocal_rank_fusion
        ...
        vec_results = await self._vector_search(
            embedding=embedding, namespace=namespace, top_k=top_k * 3,
            include_historical=include_historical, query=query, as_of=as_of,
        )
        ...
        if memories:
            idx = BM25Index([(str(m.id), m.content) for m in memories])
            bm25_ranked = idx.score(query)[: top_k * 3]

        # Collect memory objects by id (union of both arms)
        by_id: dict[str, object] = {}
        for r in vec_results:
            by_id[str(r.memory.id)] = r.memory
        for m in memories:
            by_id.setdefault(str(m.id), m)

        vec_ranking = [str(r.memory.id) for r in vec_results]
        bm25_ranking = [mid for mid, _ in bm25_ranked]
        fused = reciprocal_rank_fusion([vec_ranking, bm25_ranking], k=60)

        ranked_ids = sorted(fused.items(), key=lambda kv: kv[1], reverse=True)[:top_k]
        out: list[SearchResult] = []
        for mid, score in ranked_ids:
            mem = by_id.get(mid)
            if mem is None:
                continue
            out.append(
                SearchResult(
                    memory=mem, score=float(score), source="rrf",
                    is_current=(getattr(mem, "superseded_at", None) is None),
                )
            )
        return out
    ...
    def _require_vault(self) -> None:
        if self._vault is None:
            raise RuntimeError("Vault is not enabled. Set vault.enabled: true in memnos.yaml")
