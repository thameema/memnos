"""
memnos.vault.vault_client — Envelope encryption engine for the secrets vault.
...
KEK providers
-------------
  local           KEK is derived from MEMNOS_VAULT_KEY env var (or config
...
unchanged.  Run:  memnos vault rotate-kek  (CLI, not yet implemented).
"""
...
if TYPE_CHECKING:
    from memnos.config import VaultConfig
...
class VaultClient:
    ...
    # ------------------------------------------------------------------
    # Local mode (MEMNOS_VAULT_KEY env var / fallback_key)
    # ------------------------------------------------------------------
    ...
    def _get_local_kek(self) -> bytes:
        ...
        key_str = self._config.fallback_key or os.environ.get("MEMNOS_VAULT_KEY", "")
        if not key_str:
            raise RuntimeError(
                "Vault is enabled but MEMNOS_VAULT_KEY is not set.  "
                "Generate a key with:  python -c \"import os,base64; print(base64.urlsafe_b64encode(os.urandom(32)).decode())\"  "
                "and set it as the MEMNOS_VAULT_KEY environment variable."
            )
...
async def rotate_kek_local(
    ...
    ----------
    old_kek_str:
        Current MEMNOS_VAULT_KEY (raw key or passphrase).
    new_kek_str:
        Replacement MEMNOS_VAULT_KEY.
    arcadedb_client:
