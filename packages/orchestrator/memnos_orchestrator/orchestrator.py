"""
memnos_orchestrator.orchestrator — Main orchestration loop.
...
class Orchestrator:
    ...
    Usage
    -----
    orch = Orchestrator(config, memnos_client, task_store)
    await orch.start()
    ...
    def __init__(
        self,
        config: MemnosConfig,
        memnos_client: Any,  # MemnosClient
        task_store: TaskStore,
    ) -> None:
        self._config = config
        self._memnos_client = memnos_client
    ...
    async def start(self) -> None:
        ...
        import os
        agents_dir = os.environ.get("MEMNOS_AGENTS_DIR", "agents")
        self._router = AgentRouter(
            agents_dir=agents_dir,
            memnos_client=self._memnos_client,
        )
    ...
    async def run(
        ...
        try:
            ...
            # Best-effort episodic record — requires memnos_learning
            try:
                from memnos_learning.episode_store import EpisodeStore  # type: ignore
                from memnos_learning.extractor import SkillExtractor  # type: ignore
                from memnos_learning.models import EpisodicRecord, Outcome  # type: ignore
                from memnos_learning.skill_store import SkillStore  # type: ignore
    ...
    async def _get_past_context(self, task: str, namespace: str) -> str:
        ...
        try:
            results = await self._memnos_client.search(
