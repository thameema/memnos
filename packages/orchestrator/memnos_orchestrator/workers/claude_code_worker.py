"""
memnos_orchestrator.workers.claude_code_worker — Worker that spawns the `claude` CLI.

Writes a temporary MCP config pointing at the local memnos MCP server,
...
class ClaudeCodeWorker(BaseWorker):
    """Worker that spawns the `claude` CLI subprocess with an memnos MCP config."""
    ...
    def _build_mcp_config(self, config_path: str) -> None:
        """Write a temporary MCP config JSON file pointing at the memnos MCP server."""
            ...
            server_config = {
                "mcpServers": {
                    "memnos": {
                        ...
                        "env": {
                            "MEMNOS_NAMESPACE": self._namespace,
                        },
                    }
                }
            }
    ...
    async def run(self, task_prompt: str, system_prompt: str | None = None) -> str:
        ...
        Spawn the `claude` CLI with:
          - A temporary MCP config pointing at the memnos MCP server
