"""
memnos_mcp.tools.orchestrator_tools — MCP tool handlers for task orchestration,
...
_AGENTS_DIR_ENV = "MEMNOS_AGENTS_DIR"
...
async def handle_get_heuristics(
    ...
    try:
        ...
        logger.warning("memnos_learning not installed; returning empty heuristics")
        heuristics = []
...
async def handle_add_heuristic(
    ...
    try:
        from memnos_learning.heuristic_store import HeuristicStore  # type: ignore
        from memnos_learning.models import Heuristic  # type: ignore
        ...
        logger.warning("memnos_learning not installed; heuristic not persisted")
...
async def handle_trigger_reflection(
    ...
    try:
        ...
        model = os.environ.get("MEMNOS_REFLECTION_MODEL", "claude-haiku-4-5-20251001")
        ...
        logger.warning("memnos_learning not installed; reflection not available")
        message = "memnos_learning package not installed"
        triggered = False
...
async def handle_list_agents(
    ...
    The directory is resolved from the ``MEMNOS_AGENTS_DIR`` environment
