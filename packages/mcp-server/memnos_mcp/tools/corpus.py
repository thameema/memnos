"""
memnos_mcp.tools.corpus — MCP tool handlers for corpus constraint checking.
...
_MEMNOS_API_BASE = os.environ.get("MEMNOS_API_BASE", "http://localhost:8766")
_MEMNOS_API_KEY  = os.environ.get("MEMNOS_API_KEY", "memnos-local-dev-key")
...
async def handle_corpus_check(
    ...
    Parameters
    ----------
    client     : MemnosClient (unused — calls REST API directly for corpus endpoints)
    ...
    url = f"{_MEMNOS_API_BASE}/api/v1/corpus/{corpus_id}/check"
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as http:
            resp = await http.post(
                url,
                json=payload,
                headers={"Authorization": f"Bearer {_MEMNOS_API_KEY}"},
            )
...
async def handle_corpus_list(client) -> dict:
    ...
    url = f"{_MEMNOS_API_BASE}/api/v1/corpus/"
    try:
        async with httpx.AsyncClient(timeout=10) as http:
            resp = await http.get(url, headers={"Authorization": f"Bearer {_MEMNOS_API_KEY}"})
            resp.raise_for_status()
            corpora = resp.json()
    except Exception as exc:
