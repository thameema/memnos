"""
memnos_mcp.server — MCP server definition and entry point.
...
from memnos_mcp.tools.graph import (
    handle_get_entity,
    handle_get_related,
    handle_graph_query,
)
...
from memnos_mcp.tools.orchestrator_tools import (
...
from memnos_mcp.tools.vault import (
    handle_secret_set,
    handle_secret_get,
    handle_secret_list,
...
from memnos_mcp.tools.corpus import (
    handle_corpus_check,
    handle_corpus_list,
)
...
TOOLS: list[Tool] = [
    Tool(
        name="memory_search",
        description=(
            "Search memnos persistent memory using vector similarity, knowledge-graph "
        ...
        inputSchema={
            ...
            "properties": {
                ...
                "namespace": {"type": "string", "description": "Memnos namespace to search"},
    ...
    Tool(
        name="memory_write",
        description=(
            "Write a new entry to memnos persistent memory. "
    ...
    Tool(
        name="graph_query",
        description=(
            "Execute a read-only Cypher query against memnos's knowledge graph (Neo4j). "
    ...
    Tool(
        name="spawn_task",
        description=(
            "Fork a background worker task via the memnos orchestrator. "
    ...
    Tool(
        name="add_heuristic",
        description="Manually add a heuristic rule to the memnos learning store.",
        inputSchema={
            "type": "object",
            "properties": {
    ...
    Tool(
        ...
        description=(
            "Trigger the memnos reflection agent to distil heuristics and skills "
    ...
    Tool(
        ...
        description=(
            ...
            "(MEMNOS_AGENTS_DIR env var, default ./agents). "
    ...
    Tool(
        ...
        description=(
            "Store (or replace) an encrypted secret in the memnos vault. "
    ...
    Tool(
        ...
        description=(
            "Retrieve and decrypt a secret from the memnos vault. "
    ...
    Tool(
        name="skill_discover",
        description=(
            "Seed or refresh tool capability catalogs in memnos. "
    ...
    Tool(
        ...
        description=(
            ...
            "Use before implementing or reviewing code in gridops modules to surface applicable "
...
def create_mcp_server(client, orchestrator, config) -> Server:
    ...
    Parameters
    ----------
    client       : MemnosClient (already started)
    orchestrator : Orchestrator (already started)
    config       : MemnosConfig (for metadata / future auth)
    """
    global _EXTERNAL_TOOLS

    # Load external skill packs (non-fatal; bad packs are logged and skipped)
    try:
        from memnos_mcp.skill_packs import load_skill_packs  # noqa: PLC0415
...
async def _dispatch(
    ...
    if name == "skill_discover":
        from memnos.skill_coach.seeder import seed_tool_capabilities, seed_claude_code_capabilities
        ...
        if not community:
            return [TextContent(
                ...
                text=(
                    ...
                    "Run memnos-community detect to build communities."
                ),
            )]
        lines = [
    ...
    if name in _SKILL_PACK_HANDLERS:
        from memnos_mcp.skill_packs import call_webhook_handler  # noqa: PLC0415
...
def _load_config(config_path: str):
    """
    Load MemnosConfig from a YAML file with ${VAR} env-var expansion.
    ...
    try:
        ...
        return MemnosConfig.from_yaml(config_path)
    except ImportError:
        # Minimal stand-in so the server still starts
        logger.warning("memnos.config not importable; using raw YAML dict")
...
class _DictConfig:
    """Minimal dot-access wrapper around a plain dict for when memnos-core is absent."""
...
async def _start_services(config):
    """Instantiate and start MemnosClient + Orchestrator."""
    ...
    orchestrator = Orchestrator(config=config, memnos_client=client, task_store=TaskStore())
...
def main() -> None:
    """
    Main entry point for the ``memnos-mcp`` CLI command.

    Transport selection (in priority order):
      1. ``--transport sse`` / ``--transport stdio`` CLI flag
      2. ``MEMNOS_TRANSPORT`` environment variable
      3. Default: stdio
    """
    parser = argparse.ArgumentParser(description="memnos MCP server")
    parser.add_argument(
        "--transport",
        choices=["stdio", "sse"],
        default=None,
        help="Transport to use (stdio or sse). Overrides MEMNOS_TRANSPORT env var.",
    )
    parser.add_argument(
        "--config",
        default=None,
        help="Path to memnos YAML config file. Overrides MEMNOS_CONFIG env var.",
    )
    parser.add_argument(
        "--host",
        default=None,
    ...
    config_path = args.config or os.environ.get("MEMNOS_CONFIG", "memnos.yaml")
    transport = args.transport or os.environ.get("MEMNOS_TRANSPORT", "stdio")
    ...
    if transport == "sse":
        from memnos_mcp.transports.sse import run_sse_server  # noqa: PLC0415

        host = args.host or os.environ.get("MEMNOS_HOST", None)
        port = args.port or int(os.environ.get("MEMNOS_PORT", "0")) or None
        asyncio.run(run_sse_server(config_path=config_path, host=host, port=port))
    else:
        from memnos_mcp.transports.stdio import run  # noqa: PLC0415
