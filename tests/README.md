...
| `test_contradiction_precision.py` | issue #156: contradictions/knowledge_health count only single-valued predicates (write path's own `_supersedable` classifier), uncapped totals, ratio-graded score; present-tense passive `was`/`were` facts supersede while genuinely historical ones (year / "ago" / "back then" / standalone past-state cues) still do not — write path and reconcile path |
| `test_namespace_reconcile.py` | `memnos namespace reconcile` backfill: dry-run counts, close/dedupe, idempotency, `--limit` |
| `test_reconcile_safety.py` | `memnos namespace reconcile` execution safety (issue #155), real CLI subprocesses + real signals: SIGTERM/SIGKILL mid-walk then resume from the persisted watermark with an end state identical to an uninterrupted run; per-namespace advisory lock (held elsewhere → exit 3; two concurrent runs → exactly one works); lock contention both ways (reconcile backs off on `lock_timeout` while a writer holds rows; concurrent real `remember()` calls never wait more than ~one chunk) |
| `test_reconcile_perf.py` | `memnos namespace reconcile` lookups use `sem_hnsw` (issue #155): EXPLAIN (ANALYZE, BUFFERS) + measured old-vs-new latency on 2000 real local-embedding facts at 1536-d, and identical close/expire decisions between the old single-transaction run and the new chunked run (full-scale numbers: `benchmarks/reconcile_perf.py`) |
...
| `test_orphan_entities_index.py` | knowledge_health's orphan-entities query (issue #69): EXPLAIN-based proof the rewritten query is index-using (no seq scan on `edges`, both UNION arms hit the `(namespace, src_entity, dst_entity)` index) on a seeded multi-tenant dataset where the pre-fix OR-correlated query is 1000x+ more expensive on the SAME data — plan inspection, not just result correctness |
| `test_knowledge_health_degrade_http.py` | knowledge_health degrade-not-500 (issue #69), end-to-end over HTTP: a REAL statement_timeout cancellation (ACCESS EXCLUSIVE lock on `edges`, then on `semantic`) degrades only the signal(s) that touch the locked table, returns 200 + `degraded:true` + `degraded_reasons`, never a 5xx, with every other structural signal intact — reuses the same `record_arm_failure` helper #41 fix C's recall arms use |
