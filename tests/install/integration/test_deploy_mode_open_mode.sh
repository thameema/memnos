...
# memnos.yaml correctly (full → true, server-only → false).
set -uo pipefail
...
describe "--mode flag controls open_mode in memnos.yaml"
...
A_OPEN=$(grep -E "^  open_mode:" "$HOME/.memnos/memnos.yaml" 2>/dev/null | head -1 | awk '{print $2}')
...
B_OPEN=$(grep -E "^  open_mode:" "$HOME/.memnos/memnos.yaml" 2>/dev/null | head -1 | awk '{print $2}')
...
C_OPEN=$(grep -E "^  open_mode:" "$HOME/.memnos/memnos.yaml" 2>/dev/null | head -1 | awk '{print $2}')
