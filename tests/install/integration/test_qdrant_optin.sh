...
# MEMNOS_VECTOR_BACKEND=qdrant and compose is invoked with --profile qdrant.
set -uo pipefail
...
echo "QDRANT_IN_ENV=$(grep -q '^MEMNOS_VECTOR_BACKEND=qdrant' "$HOME/.memnos/.env" && echo yes || echo no)"
...
assert_eq "$(get QDRANT_IN_ENV)" "yes" "MEMNOS_VECTOR_BACKEND=qdrant in .env"
