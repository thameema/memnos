...
# the memnos section, and is idempotent — re-running does NOT duplicate.
set -uo pipefail
...
printf 'Y\n' | bash /tmp/install-client.sh --server http://localhost:8766 --key memnos-test-1 >/tmp/p1.log 2>&1
echo "P1_EXIT=$?"
ls "$HOME/.claude/CLAUDE.md".before-memnos-* >/dev/null 2>&1 && echo "P1_BACKUP=yes" || echo "P1_BACKUP=no"
grep -q "lots of custom workflow notes" "$HOME/.claude/CLAUDE.md" && echo "P1_USER_PRESERVED=yes" || echo "P1_USER_PRESERVED=no"
grep -q "memnos — Persistent Memory MCP" "$HOME/.claude/CLAUDE.md" && echo "P1_MEMNOS_APPENDED=yes" || echo "P1_MEMNOS_APPENDED=no"

# Phase 2: re-run — answer Y to overwrite prompt
echo Y | bash /tmp/install-client.sh --server http://localhost:8766 --key memnos-test-2 >/tmp/p2.log 2>&1
COUNT=$(grep -c "memnos — Persistent Memory MCP" "$HOME/.claude/CLAUDE.md")
echo "P2_MEMNOS_COUNT=$COUNT"
SCENARIO
...
assert_eq "$(get P1_MEMNOS_APPENDED)" "yes" "memnos section appended"
assert_eq "$(get P2_MEMNOS_COUNT)" "1" "memnos section NOT duplicated on re-run"
