...
#   • Actually wipe ~/.memnos/{arcadedb,qdrant,.env,memnos.yaml} on 'yes'
#   • Generate a new MEMNOS_API_KEY / MEMNOS_VAULT_KEY
#
# Upgrade must:
#   • NEVER touch arcadedb data
#   • Preserve every value in .env (API key, vault key, ArcadeDB password)
#   • Preserve the existing open_mode in memnos.yaml
set -uo pipefail
...
ORIG_API_KEY=$(grep "^MEMNOS_API_KEY=" "$HOME/.memnos/.env" | cut -d= -f2)
ORIG_VAULT_KEY=$(grep "^MEMNOS_VAULT_KEY=" "$HOME/.memnos/.env" | cut -d= -f2)
ORIG_OPEN_MODE=$(grep -E "^  open_mode:" "$HOME/.memnos/memnos.yaml" | awk '{print $2}')
# Plant a fake arcadedb db file so we can tell if it got wiped
mkdir -p "$HOME/.memnos/arcadedb/memnos"
echo "fake-memory-data" > "$HOME/.memnos/arcadedb/memnos/marker.txt"
...
NEW_API_KEY=$(grep "^MEMNOS_API_KEY=" "$HOME/.memnos/.env" | cut -d= -f2)
NEW_VAULT_KEY=$(grep "^MEMNOS_VAULT_KEY=" "$HOME/.memnos/.env" | cut -d= -f2)
NEW_OPEN_MODE=$(grep -E "^  open_mode:" "$HOME/.memnos/memnos.yaml" | awk '{print $2}')
...
[ -f "$HOME/.memnos/arcadedb/memnos/marker.txt" ] && echo "P2_DATA_PRESERVED=yes" || echo "P2_DATA_PRESERVED=no"
...
[ -f "$HOME/.memnos/arcadedb/memnos/marker.txt" ] && echo "P3_DATA_KEPT=yes" || echo "P3_DATA_KEPT=no"
[ -f "$HOME/.memnos/.env" ] && echo "P3_ENV_KEPT=yes" || echo "P3_ENV_KEPT=no"

# ── Phase 4: Fresh (option 2) — user types 'yes' → MUST wipe + regen
printf '2\nyes\n\n\n\n\n\nN\n' | bash /tmp/install-server.sh --version master --mode full >/tmp/p4.log 2>&1
echo "P4_EXIT=$?"
[ ! -f "$HOME/.memnos/arcadedb/memnos/marker.txt" ] && echo "P4_DATA_WIPED=yes" || echo "P4_DATA_WIPED=no"
NEW2_API_KEY=$(grep "^MEMNOS_API_KEY=" "$HOME/.memnos/.env" 2>/dev/null | cut -d= -f2)
...
assert_eq "$(get P2_API_KEY_PRESERVED)"   "yes" "upgrade: MEMNOS_API_KEY unchanged"
assert_eq "$(get P2_VAULT_KEY_PRESERVED)" "yes" "upgrade: MEMNOS_VAULT_KEY unchanged"
...
assert_eq "$(get P4_NEW_KEY)"     "yes" "fresh+'yes': brand-new MEMNOS_API_KEY generated"
