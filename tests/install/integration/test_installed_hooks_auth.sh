...
bash /tmp/install-client.sh --server http://localhost:8766 --key memnos-AUTH-TEST --namespace personal:me >/tmp/cli.log 2>&1
...
# Per-hook check
for h in memnos-inject.sh memnos-git-write.sh memnos-precompact.sh memnos-session-write.sh memnos-heartbeat.py; do
  f="$HOOKS_DIR/$h"
  if [ -f "$f" ]; then
...
# Slash command
if [ -f "$CMDS_DIR/memnos.md" ]; then
  grep -q "X-API-Key" "$CMDS_DIR/memnos.md" && echo "SLASH_HAS_XAPI=yes" || echo "SLASH_HAS_XAPI=no"
  grep -q "Authorization.*Bearer" "$CMDS_DIR/memnos.md" && echo "SLASH_HAS_BEARER=yes" || echo "SLASH_HAS_BEARER=no"
else
...
assert_eq "$(get SLASH_HAS_XAPI)" "no" "/memnos slash command does NOT use X-API-Key"
assert_eq "$(get SLASH_HAS_BEARER)" "yes" "/memnos slash command uses Bearer"
