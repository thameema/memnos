...
#
# Each test brings up an isolated memnos stack with:
#   • Separate project name (memnos-e2e) so containers don't collide with
#     the developer's existing memnos install
#   • Test-only ports: 18765 (MCP/SSE), 18766 (REST API), 12480 (arcadedb)
#   • Test-only data dir: /tmp/memnos-e2e-data
#   • Test-only source clone: /tmp/memnos-e2e-src
#
# Setup is cached across tests in the same run: if the memnos-e2e stack is
# already up and healthy from a prior test, e2e_up reuses it.

E2E_PROJECT="memnos-e2e"
E2E_SRC="/tmp/memnos-e2e-src"
E2E_DATA="/tmp/memnos-e2e-data"
E2E_API_PORT="18766"
E2E_MCP_PORT="18765"
...
e2e_key() {
  grep '^MEMNOS_API_KEY=' "$E2E_DATA/.env" | cut -d= -f2
}
...
e2e_up() {
  ...
  echo "[e2e] bringing up isolated memnos stack at :${E2E_API_PORT}/:${E2E_MCP_PORT}..."
...
MEMNOS_API_KEY=${KEY}
MEMNOS_VAULT_KEY=${VAULT}
ANTHROPIC_API_KEY=
OPENAI_API_KEY=
MEMNOS_EMBED_MODE=local
MEMNOS_DATA_DIR=${E2E_DATA}
MEMNOS_CONFIG_FILE=${E2E_DATA}/memnos.yaml
EOF
  chmod 600 "$E2E_DATA/.env"

  cp "$E2E_SRC/memnos.yaml.example" "$E2E_DATA/memnos.yaml"
  ...
  # memnos-arcadedb / memnos / memnos-qdrant — without overriding those, the
  # 'compose up' fails with "name already in use" when the dev stack exists.
  cat > "$E2E_SRC/docker-compose.override.yml" <<EOF
services:
  arcadedb:
    container_name: memnos-e2e-arcadedb
    ports: !override
      - "${E2E_ARCADE_PORT}:2480"
  memnos:
    container_name: memnos-e2e
    ports: !override
      - "${E2E_MCP_PORT}:8765"
      - "${E2E_API_PORT}:8766"
  qdrant:
    container_name: memnos-e2e-qdrant
    ports: !override []
EOF

  echo "[e2e] building memnos image (3-5 min on cold cache)..."
  e2e_compose build memnos >/dev/null 2>&1 || { echo "[e2e] build failed"; e2e_compose logs memnos | tail -20; return 1; }

  echo "[e2e] starting services..."
  e2e_compose up -d >/dev/null 2>&1 || { echo "[e2e] up failed"; return 1; }

  echo "[e2e] waiting for memnos to be healthy..."
  local i=0
  while ! curl -sf "${E2E_API}/api/v1/admin/health" -H "Authorization: Bearer ${KEY}" >/dev/null 2>&1; do
    sleep 3; i=$((i+1)); echo -ne "\r  waiting ${i}*3s..."
    [ $i -ge 40 ] && { echo ""; echo "[e2e] memnos never became healthy"; e2e_compose logs memnos | tail -30; return 1; }
  done
  echo ""
