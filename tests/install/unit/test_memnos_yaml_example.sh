#!/usr/bin/env bash
# Unit: memnos.yaml.example must use ${ARCADEDB_HOST:-localhost} interpolation,
...
YAML="${REPO_ROOT}/memnos.yaml.example"
describe "memnos.yaml.example uses env-var interpolation"
assert_file "$YAML"
...
# Intentionally NOT using PyYAML: memnos.yaml has bash-style env-var
...
# .env.example sanity — every variable memnos.yaml interpolates should be
# defined (or commented) in .env.example.
ENV_EX="${REPO_ROOT}/.env.example"
describe ".env.example references match memnos.yaml.example"
assert_file "$ENV_EX"
for var in ARCADEDB_PASSWORD MEMNOS_API_KEY MEMNOS_VAULT_KEY; do
