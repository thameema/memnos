    ...
    def _openai(self, body, marker):
        ...
        if body.get("stream"):
            ...
            chunks = [
                ...
                {"choices": [{"delta": {"content": "PROJ-467 is the anchor."}, "finish_reason": None}]},
    ...
    def _anthropic(self, body, marker):
        if body.get("stream"):
            ...
            evs = [
                ...
                {"type": "content_block_delta", "delta": {"type": "text_delta", "text": "PROJ-471 and PROJ-472 raised."}},
...
def main():
    ...
    check("openai stream: accumulated reply captured with identifier",
          wait_capture(2) and "PROJ-467" in remembered[1]["text"])
    ...
    check("anthropic stream: relayed + captured",
          st == 200 and wait_capture(2) and "PROJ-471" in remembered[1]["text"])
