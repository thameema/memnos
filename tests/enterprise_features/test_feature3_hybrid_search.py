...
_CORPUS = [
    "Customer Acme Health runs Innovaccer for population health analytics",
    "Decision: replace MongoDB with PostgreSQL for OLAP workloads",
    "Bug PROJ-413: scheduler-service fails on retry after 503 from upstream",
...
class TestBM25Mode:
    def test_bm25_matches_exact_ticket_id(self, http, seeded_ns: str):
        results = _search(http, seeded_ns, "PROJ-413", mode="bm25")
        assert results, "bm25 returned no results"
        assert "PROJ-413" in results[0]["content"], (
...
class TestRRFMode:
    def test_rrf_returns_results_for_simple_query(self, http, seeded_ns: str):
        results = _search(http, seeded_ns, "PROJ-413", mode="rrf")
