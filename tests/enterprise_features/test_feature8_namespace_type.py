...
Regression case — the cms57f → hdig leak we just hit in production:
  * Seed 'reference' namespace with a CMS-57F-style memory.
  * Seed 'work' namespace with an hdig-style memory that mentions FHIR.
  * Default search for an FHIR-related hdig query must NOT return the
    CMS-57F result.
...
# ---------------------------------------------------------------------------
# Regression — exact cms57f → hdig leak we hit in production
# ---------------------------------------------------------------------------

class TestCmsHdigLeakRegression:
    """The exact scenario that broke developer trust:
       a CMS-0057-F reference doc bled into an hdig engineering query."""

    def test_cms57f_does_not_leak_into_hdig(self, http):
        hdig_ns = _new_ns("hdig")
        cms_ns = _new_ns("cms57f")

        # hdig (work) — engineering memory mentioning FHIR
        hdig_mem = _write(
            http, hdig_ns,
            "HPTE-445 fix: ClaimResponse FHIR profile citation reverted; "
        ...
        # Developer's query — pure hdig context, no expectation of policy text
        results = http.get(
            "/api/v1/memory/search",
            params={"q": "HPTE-445 FHIR ClaimResponse fix", "ns": "all",
                    "mode": "rrf", "top_k": 5},
        ).json()
        ids = {r["id"] for r in results}

        # hdig memory is what they wanted
        assert hdig_mem["id"] in ids, (
            f"hdig memory missing from results: {results}"
        )
        # cms57f memory MUST NOT leak — this was the credibility-breaking bug
        assert cms_mem["id"] not in ids, (
            f"REGRESSION: cms57f reference memory leaked into hdig query. "
    ...
    def test_opting_in_surfaces_cms57f_for_grounding(self, http):
        ...
        hdig_ns = _new_ns("hdig-opt")
        cms_ns = _new_ns("cms57f-opt")
        hdig_mem = _write(http, hdig_ns, f"HPTE-445 {tag} fix ClaimResponse FHIR profile")
        ...
        ids = {r["id"] for r in results}
        assert hdig_mem["id"] in ids, f"hdig seed missing; results={ids}"
